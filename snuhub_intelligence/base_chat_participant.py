...
from snuhub_intelligence.api import ChatCommand, ChatParticipant, ChatRequest, ChatResponse, MarkdownData, ProgressData, Tool, ToolPreInvokeResponse
from snuhub_intelligence.prompts import Prompts
import base64
import logging
from snuhub_intelligence.built_in_toolsets import built_in_toolsets

from snuhub_intelligence.util import extract_llm_generated_code
    ...
    async def handle_tool_call(self, request: ChatRequest, response: ChatResponse, tool_context: dict, tool_args: dict) -> str:
        ...
        for cell_source in cell_sources:
            ...
            if cell_type == 'markdown':
                ...
                ui_cmd_response = await response.run_ui_command('snuhub-intelligence:add-markdown-cell-to-notebook', {'markdown': source, 'path': file_path})
            elif cell_type == 'code':
                source = cell_source.get('source', '')
                ui_cmd_response = await response.run_ui_command('snuhub-intelligence:add-code-cell-to-notebook', {'code': source, 'path': file_path})
...
class AddMarkdownCellToNotebookTool(Tool):
    ...
    async def handle_tool_call(self, request: ChatRequest, response: ChatResponse, tool_context: dict, tool_args: dict) -> str:
        ...
        ui_cmd_response = await response.run_ui_command('snuhub-intelligence:add-markdown-cell-to-notebook', {'markdown': source, 'path': notebook_file_path})
...
class AddCodeCellTool(Tool):
    ...
    async def handle_tool_call(self, request: ChatRequest, response: ChatResponse, tool_context: dict, tool_args: dict) -> str:
        ...
        ui_cmd_response = await response.run_ui_command('snuhub-intelligence:add-code-cell-to-notebook', {'code': source, 'path': notebook_file_path})
...
class PythonTool(AddCodeCellTool):
    ...
    async def handle_tool_call(self, request: ChatRequest, response: ChatResponse, tool_context: dict, tool_args: dict) -> str:
        ...
        ui_cmd_response = await response.run_ui_command('snuhub-intelligence:add-code-cell-to-notebook', {'code': code, 'path': tool_context.get('file_path')})
...
class BaseChatParticipant(ChatParticipant):
    ...
    async def handle_ask_mode_chat_request(self, request: ChatRequest, response: ChatResponse, options: dict = {}) -> None:
        ...
        if request.command == 'newNotebook':
            ...
            ui_cmd_response = await response.run_ui_command('snuhub-intelligence:add-markdown-cell-to-notebook', {'markdown': markdown, 'path': file_path})
            ui_cmd_response = await response.run_ui_command('snuhub-intelligence:add-code-cell-to-notebook', {'code': code, 'path': file_path})
            ...
            ui_cmd_response = await response.run_ui_command('snuhub-intelligence:open-configuration-dialog')
